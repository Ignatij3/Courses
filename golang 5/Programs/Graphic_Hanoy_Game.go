...
func DrawArrow(pos int32, r, g, b uint8) {
	...
	renderer.SetDrawColor(r, g, b, 255)
	if (*frac).A == 16 && (*frac).B == 9 {
		arrow = sdl.Rect{(((winWidth * 7) / 24) * pos) + (winWidth*77)/384, 1, winWidth / 64, (winHeight * 5) / 54}
	}
...
func (lifted *Ring) Push(first, second, third *[]Ring, current int32) {
	switch current {
	case 0:
		if len(*first) == 0 || lifted.n < (*first)[len(*first)-1].n {
			*first = append(*first, *lifted)
			*lifted = Ring{0, 0}
		}
	case 1:
		if len(*second) == 0 || lifted.n < (*second)[len(*second)-1].n {
			*second = append(*second, *lifted)
			*lifted = Ring{0, 0}
		}
	case 2:
		if len(*third) == 0 || lifted.n < (*third)[len(*third)-1].n {
			*third = append(*third, *lifted)
			*lifted = Ring{0, 0}
		}
	}
